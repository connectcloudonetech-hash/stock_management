...
import { ArrowUpCircle, Check, Loader2, AlertTriangle, Banknote, Tag, ChevronDown, Edit3 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id) {
        ...
        await stockService.carryOut(payload);
      }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-10">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-xl space-y-6">
        {error && <div className="bg-rose-50 border border-rose-200 text-rose-700 px-4 py-3 rounded-2xl flex items-center space-x-2 font-bold text-sm"><AlertTriangle size={18} /> <span>{error}</span></div>}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-700 uppercase">Date</label>
            <input type="date" required value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 font-bold" />
          </div>
          <div className="space-y-2">
            ...
            {isAddingNewCustomer ? (
              ...
              <select required value={formData.customer_id} onChange={(e) => setFormData({...formData, customer_id: e.target.value})} className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 font-bold appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236b7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.5em_1.5em] bg-[right_1rem_center] bg-no-repeat uppercase">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-4 focus:ring-slate-900/10 transition text-slate-900 font-bold appearance-none uppercase"
            >
          ...
          {formData.category === 'OTHERS' && (
            <div className="relative mt-3 animate-in slide-in-from-top-2 duration-300">
              <Edit3 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600" size={18} />
              <input 
                required
                type="text" 
                ...
                className="w-full pl-12 pr-4 py-4 bg-slate-100 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-slate-900 transition text-slate-900 font-bold uppercase"
              />
            </div>
          )}
        </div>
        ...
        <div className="space-y-2">
          <label className="text-sm font-bold text-slate-700 uppercase">Carry OUT Amount (INR)</label>
          <div className="relative">
        ...
        <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-black shadow-xl flex items-center justify-center transition-all active:scale-[0.98] uppercase">
